...
export default function mergeSourceMap(
  ...
  const result = (mergedGenerator as any).toJSON();
...
function eachOverlappingGeneratedOutputRange(
  ...
  callback: (item: ResolvedGeneratedRange) => any,
) {
...
function eachInputGeneratedRange(
  ...
  callback: (
    generated: ResolvedGeneratedRange,
...
interface ResolvedMappings {
  file?: string;
  sourceRoot?: string;
  sources: ResolvedFileMappings[];
}
interface ResolvedFileMappings {
  source: ResolvedSource;
  mappings: OriginalMappings;
}
type OriginalMappings = OriginalMapping[];
interface OriginalMapping {
  original: ResolvedOriginalRange;
  generated: ResolvedGeneratedRange[];
}
...
interface ResolvedOriginalRange {
  line: number;
  columnStart: number;
  columnEnd: number;
  name: string | null;
}
interface ResolvedGeneratedRange {
  line: number;
  columnStart: number;
  columnEnd: number;
}
...
function buildMappingData(map: SourceMap): ResolvedMappings {
  const consumer = new sourceMap.SourceMapConsumer(({
    ...
    // final map at the end insteaSd.
    sourceRoot: (null as any),
  } as any));
      ...
      sourceData.mappings.push({
        ...
        generated: consumer
          ...
          .map(item => ({
            ...
            // to add 1 to it.
            columnEnd: (item as any).lastColumn + 1,
          })),
      });
    },
    null,
